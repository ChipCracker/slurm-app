...
public class SSH {
    ...
    /// Authenticate using a private key held **in memory** (PEM/OpenSSH text),
    /// without any file on disk. The public key is derived from the private key
    /// by the crypto backend. Use this on iOS / in GUIs where there is no
    /// `~/.ssh`, no `.pub` file, and no TTY for a passphrase prompt.
    ///
    /// - Parameters:
    ///   - username: the username to login with
    ///   - privateKeyData: the private key contents (PEM or OpenSSH format)
    ///   - passphrase: the passphrase encrypting the key; nil if unencrypted
    /// - Throws: SSHError if authentication fails
    public func authenticate(username: String, privateKeyData: String, passphrase: String? = nil) throws {
        try session.authenticate(username: username, privateKeyData: privateKeyData, passphrase: passphrase)
    }
